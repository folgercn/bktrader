...
func init() {
	...
	runtimeStartCmd.Flags().String("kind", "signal", "runtime 类型 (signal, live-session, paper-session)")
	runtimeStartCmd.Flags().Bool("confirm", false, "确认执行启动操作")
	runtimeStartCmd.Flags().String("reason", "", "启动原因；必填")
	runtimeStopCmd.Flags().String("kind", "signal", "runtime 类型 (signal, live-session, paper-session)")
