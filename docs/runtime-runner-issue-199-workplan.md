...
- ✅ 已完成：`SignalRuntimeEventConsumer` handler、NATS pull durable consumer `live-evaluation`、success ack / failure nak、stale event drop、live-runner / monolith 启动 consumer、复用现有 live evaluation fanout。
- ✅ 已覆盖测试：consumer config、event 触发 live evaluation happy path、duplicate delivery 幂等、stale event ack/drop、failure 不 ack。
