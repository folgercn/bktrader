...
- after source gating passes, the platform now calls a strategy-engine-level `signal evaluation` hook:
  ...
  - the engine also emits a strategy-facing `signalKind` such as `initial-entry`, `initial-entry-watch`, `sl-reentry`, `sl-reentry-watch`, `pt-reentry`, `pt-reentry-watch`, `hold-long`, `hold-short`, `protect-exit`, `protect-exit-watch`, `protect-exit-near`, `risk-exit`, `risk-exit-watch`, or `risk-exit-near`
