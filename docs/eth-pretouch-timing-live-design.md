...
- Gate 证据：
  ...
  strict 15bp proxy `28.970948%` 跑赢 `lead_adverse10_exact`，strict 20bp proxy
...
- 结论：
  ...
  - `severe` / 15bp 最好只有 `22.222649%`，低于 `lead_adverse10_exact`，仍必须作为 thin-book kill stress。
...
- Conditional 结果：
  ...
  - `severe` / 15bp 最优为 `21.231073%`，仍低于 `lead_adverse10_exact`，因此 severe/thin-book
    必须阻断 risk-on sizing。
