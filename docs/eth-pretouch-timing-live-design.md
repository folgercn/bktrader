...
若 T3 RF artifact 缺失或 lead RF probability 特征不可用，T3 overlay 不扩大仓位，回落 fixed
`0.080` ETH 并在 `pretouchShadowOverlayQualitySizing.status` 写入 `model_missing` /
...
`no_model_loaded`，不入场，也不会退化为 fixed sizing。T3 overlay quality 模型不可用时，仅该
quality multiplier 回落到 `1.0`，继续按 fixed overlay 采样，并在 metadata 标记原因。
