...
默认指当前 risk-on shadow bundle：**原 2026-05-15 ETH pretouch lead + lead `1.5x`
条件仓位放大 + T3 overlay `2.0x`**。原来的 2026-05-15 production-aligned lead 以后在本文档中
统一称为 `base lead` / `original lead`；只有做历史对照或保守基线时才单独引用它。

| 维度 | 固化口径 |
| --- | --- |
| Current research lead | `eth_pretouch_lead_1p5x_t3_overlay_2p0_20260519` |
| 组合定义 | `base lead` + `pretouchShadowLeadScale=1.5` + `pretouchShadowOverlayScale=2.0` |
...
| Lead sizing | `productionSuggestedQuantity` 先由 RF probability / cost penalty 生成，再在 shadow guard 通过时乘 `1.5x` |
| T3 overlay sizing | 独立 `entry-t3-overlay` event source，`pretouchShadowOverlayBaseShare=0.40`、`pretouchShadowOverlayScale=2.0` |
...
`base_lead_adverse10_exact`，而是 `base lead + lead 1.5x + T3 overlay 2.0x`。下表先保留 base lead
...
| --- | ---: | --- | --- |
| `current_research_lead_strict10` | `35.521555%` | `lead_scale=1.5`、`overlay_scale=2.0`、strict impact proxy、10bp overlay/slippage pressure | 较低成交压力下的进攻预期，不单独作为 promotion 依据。 |
| `current_research_lead_strict15` | `28.970948%` | `lead_scale=1.5`、`overlay_scale=2.0`、strict impact proxy、15bp overlay/slippage pressure | 当前 shadow target 的主对照；相对 `base_lead_adverse10_exact` 的 conservative lift 约 `5.999300pp`。 |
| `current_research_lead_strict20` | `22.420341%` | `lead_scale=1.5`、`overlay_scale=2.0`、strict impact proxy、20bp overlay/slippage pressure | kill-stress 边界；只剩接近 base adverse10 的收益，不应被当成充分 lift。 |
| `current_research_lead_severe15` | `21.231073%` | `lead_scale=1.5`、`overlay_scale=2.0`、severe thin-book proxy、15bp pressure | thin-book kill-stress；severe profile 必须阻断 risk-on sizing。 |
...
lead scale 临时提升到 `1.25x`”作为下一轮 research candidate。后续 risk-appetite sweep 已把
...
- 在仅用当前 live pre-submit guard 重算的数量缩放矩阵里，当前 shadow target `1.5x` lead scale 为
...
- Gate 证据：
  当前 `lead 1.5x / overlay 2.0x` shadow target 的 live telemetry 为 `6/6` pass，
...
| `testnet_shadow_collect` | `sandbox=true`，ETHUSDT 1h，允许显式 risk-on shadow 提交 1.5x lead quantity，并允许 T3 overlay 2.0x 生成独立 testnet entry proposal，收集 decision/order/fill/depth telemetry | mainnet、全局默认 sizing 放大、跨品种推广、把 T3 overlay 当成 mainnet/live candidate、一次 decision 多订单派发 | live samples `>=30` 或触发 kill gate |
...
| 当前 submitted sizing | **testnet shadow 条件放大** | Lead base `productionSuggestedQuantity = pretouchBaseOrderQuantity * pretouchBaseShare * clip(rf_probability * 2, 0, 2) * costPenalty`，通过 risk-on guard 后为 `1.5x`；T3 overlay 为独立 `entry-t3-overlay` intent，默认 `pretouchBaseOrderQuantity * 0.40 * 2.0`，并受 `pretouchShadowMaxSubmittedQuantity=0.40` 限制。 |
...
| Model | `data/pretouch_model.json` / `20260515_v1` 只用于 lead 的 Go-native DT3 timing classifier + RF probability；T3 overlay 当前不跑 RF/timing 模型，按 research `all_speed_abs_ge035` gate 采样 |
| Model features | `roundtrip_cost_atr`、`prev1_range_atr`、`prev1_close_pos_side`、`level_to_signal_open_atr`、`touch_extension_atr`、`speed_300s_atr`、`eff_300s`、`pre_touch_seconds` |
...
| Shadow hard caps | `pretouchShadowLeadScale <= 1.5`、`pretouchShadowOverlayScale <= 2.0`、`pretouchShadowOverlayBaseShare <= 0.40`、`pretouchShadowMaxSubmittedQuantity <= 0.40`；`testnet_shadow_collect` 下 `pretouchBaseOrderQuantity` 也会被 cap 到 max submitted quantity。 |
...
`pretouchShadowMaxSubmittedQuantity` 硬限制，模板默认值为 `0.40` ETH。这个 cap 高于当前
`baseQuantity=0.100`、`pretouchBaseShare=0.80`、RF multiplier 最大 `2.0`、lead scale `1.5`
对应的 research max lead quantity `0.240` ETH，避免把 high-RF lead 正常放大量截回 `0.20`；
...
| 1.5x lead scale + 2.0x overlay strict impact proxy | - | `35.521555%` | `28.970948%` | `22.420341%` | 当前 risk-on shadow target；15bp 跑赢基线，20bp 回到基线附近 |
...
- 主候选不是追求 base lead 的 `30%+` same-close，而是在 `15bp` strict impact proxy 下保持高于
  `base_lead_adverse10_exact=22.971648%`。
- 当前可期待的 sizing-lift 净效果约为 `28.970948% - 22.971648% = 5.999300pp`
...
| Gate | 当前状态 | 结论 |
| --- | --- | --- |
| Research lead 身份已锁定 | base event/model + `lead 1.5x` + `T3 overlay 2.0x` + testnet shadow guard 已写入本文档 | pass |
| 保守基线可复算 | `base_lead_adverse10_exact=22.971648%`，62 trades，0 negative months | pass |
| Strict proxy 有 lift | strict 15bp `28.970948%` > baseline | pass |
| Kill-stress 没被误通过 | severe 15bp `21.231073%` fail，strict 20bp `22.420341%` 不再提供足够 lift | pass |
| Live telemetry 方向 | `1.5x` 当前样本 `6/6` combined pass | pass for shadow |
...
2. `1.5x` shadow-computed quantity 的 combined pass ratio 必须保持 `100%`。
...
- 增加或复用 metadata 字段记录 current submitted quantity、shadow `1.5x` quantity、T3 overlay base/share/scale/submitted quantity、scaled top-depth coverage、
...
- `1.5x` lead quantity 只允许在 testnet shadow risk-on guard 通过时真实提交；mainnet 和非 sandbox 必须保持 production sizing。
