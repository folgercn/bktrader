...
## 当前 research lead / testnet direct target

截至 2026-05-29，后续文档、research 扩展和生产排障中，未特别说明时，`research lead`
默认指当前 testnet direct bundle：**原 2026-05-15 ETH pretouch lead + lead `0.20..0.40 ETH`
quantity-band 条件仓位放大 + relaxed T3 overlay `0.20..0.40 ETH` RF/cost quality sizing +
...
| 维度 | 固化口径 |
| --- | --- |
| Current research lead | `lead_q020_q040_plus_t3_relaxed_deterministic_stop_gate_20260529` |
| 组合定义 | `base lead` + `pretouchShadowLeadQuantityBandSizing=true` (`0.20..0.40 ETH`) + `pretouchShadowT3StructureMode=prev3_dominates` + `pretouchShadowOverlayQualitySizing=true` (`0.20..0.40 ETH`) + T3 deterministic stop gate selector/action |
...
2026-05-29 更新后，`research lead` 默认不再等同于 `base_lead_same_close`、单独的
`base_lead_adverse10_exact`，也不再只等同于 PR447 的 T3 RF/cost quantity-band sizing。当前口径是
`base lead + lead 0.20..0.40 ETH quantity-band testnet direct sizing + relaxed T3 overlay
0.20..0.40 ETH RF/cost quantity-band sizing + T3 deterministic stop-gate lifecycle overlay`。下表先保留 base lead
...
| --- | ---: | --- | --- |
| `lead_quantity_0p20_0p40_adverse10` | `61.070916%` | 固定 canonical lead adverse10 ledger，只把 submitted lead quantity 按 testnet direct 公式映射到 `0.20..0.40 ETH` | 正式 T2 quantity-band 线性 notional 回测口径；相对 `base_lead_adverse10_exact` lift `+38.099268pp`，相对旧 `lead 1.5x` lift `+26.613444pp`。 |
...
2026-05-29 的 T3 lifecycle research lead 不是新增 entry event source，也不是替换 T3 RF/cost
quality sizing。它只是在 relaxed T3 overlay event 已经入场后，用一个可解释 deterministic selector 决定是否切换
出场生命周期：
...
当前 relaxed event pool 证据：

| Policy | Overlay | Worst Month | Neg Months | Max DD | Hard3 Events | Worst Trade | P90 Hold | Worst MAE |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Relaxed q020-q040 baseline | `91.360615%` | `-4.498330%` | `2` | `-6.528768%` | `0` | - | - | - |
| Relaxed deterministic gate + selected hard-stop action | `194.323156%` | `-3.357137%` | `1` | `-7.827585%` | `24` | - | - | - |

PR447 strict-pool stability checks remain historical support for the selector/action split, but no longer define the
...
因此当前 live/testnet direct 对齐前的 research lead 是 **relaxed T3 event pool + deterministic selector +
selected hard-stop action + `0.20..0.40 ETH` direct quantity band**：selector 是 deterministic gate，
action 是 `delay_trailing_updates_79m + hard_stop_atr_3.0`。

#### 预期收益与风险预算

这些收益数字是 **research / proxy 预期**，不是 testnet direct 实盘收益承诺：
...
| T3 RF/cost 0.75-1.25x overlay + exact lead | - | `35.945786%` | - | - | 历史小 band 对照；T3 专用 walk-forward RF/cost sizing 比 fixed overlay `+1.571506pp`，但数量只有 `0.06..0.10` ETH，已不是当前 testnet direct target。 |
| T3 RF/cost 0.20-0.40 ETH overlay + exact lead | - | `68.610750%` | - | - | 历史 PR447 aggressive sizing 对照；overlay leg `45.639102%`，delta vs fixed `+34.236470pp`，worst month `-3.088880%`，DD `-4.602568%`。 |
...
| T2 lead 0.20-0.40 + T3 RF/cost 0.20-0.40 | - | `106.710018%` | - | - | PR447 risk-on historical headline；按月 additive bundle，worst month `-1.464655%`，event-order DD `-4.682954%`。 |
...
- 任一 testnet direct order 的 `sandbox=false`，立即停止本路线并排查 session/template。
...
当前阶段实现 testnet direct risk-on lead sizing 和 T3 overlay entry proposal，不实现 mainnet sizing 放大：
...
- 增加或复用 metadata 字段记录 current submitted quantity、lead quantity band score/min/max、legacy shadow `1.5x` quantity、T3 overlay base/share/scale/quality multiplier/submitted quantity、scaled top-depth coverage、
  slippage headroom、spread/book-age/source-divergence、submitted before/after testnet direct sizing 和 block reason。
- 仅当 risk-on 未显式关闭、live 语义、账户 binding `sandbox=true`、`executionMode=rest`、
  testnet direct pre-submit guard 通过时，submitted proposal quantity 才进入 `0.20..0.40 ETH` lead quantity band；否则等于当前 production sizing。
...
- T3 overlay testnet direct 当前只接 initial entry proposal；research 中的 T3 reentry schedule 后续腿仍未提升为 Go live 行为。
  T3 deterministic stop-gate lifecycle overlay 已作为当前 live/testnet direct 对齐候选，但实现时必须保持 selector/action 分层：
...
| `pretouchShadowLeadQuantityBandSizing` | `true` in testnet direct template |
| `pretouchShadowLeadQuantityMinQuantity` / `MaxQuantity` | `0.20` / `0.40` ETH |
| `pretouchShadowT2StaticDownsize` | `false` in testnet direct template |
| `pretouchShadowOverlayQualitySizing` | `true` in testnet direct template |
| `pretouchShadowOverlayQualityFallbackSubmit` | `false` in testnet direct template；只有显式打开时才允许 T3 quality model 缺失/feature failed 后提交 fixed overlay fallback |
...
bk-live-eth-pretouch-timing
  ...
  -> PretouchModelBundle (T3 overlay RF/cost JSON model, testnet direct only)
...
- Lead `0.20..0.40 ETH` quantity band 只允许在 testnet direct risk-on guard 通过时真实提交；mainnet 和非 sandbox 必须保持 production sizing。
