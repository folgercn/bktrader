...
2026-05-19 对照当前 live 代码后，固定以下口径，避免后续把 `research lead`、`same_close`、
`adverse10` 和 walk-forward/monthly gate 结果混用：

| 口径 | Calendar sum | Worst month | Negative months | Trades | 说明 |
| --- | ---: | ---: | ---: | ---: | --- |
| `lead_same_close` | `30.217222%` | `2.358319%` | `0` | `62` | 不加 adverse/slippage 压力的 lead replay；“research lead 30%+”指这个口径。 |
| `lead_adverse10_exact` | `22.971648%` | `1.395821%` | `0` | `62` | 同一 lead、同一 production template exit contract 下的 next-second adverse + 10bps 压力口径；作为保守主对照。 |
...
`lead_adverse10_exact`，应作为 kill-stress。
...
- 线性 lead-scale what-if 显示，`1.25x` lead scale + 当前 `2.0x` overlay、capacity `2.0`
...
- 当前更干净的下一步不是继续拉大 T3 overlay，而是对 `1.25x` lead-scale + 2.0x overlay 做
...
- 当前 `2.0x` overlay + `1.25x` lead scale + capacity `2.0`：
  ...
  - `strict_top1p2_active1p0` / 20bp: `22.278881%`，低于 `lead_adverse10_exact`，按 kill-stress 失败
...
因此仓位提升的当前判断是：不得无条件放大默认仓位；只保留“depth/impact gate 通过时，把
lead scale 临时提升到 `1.25x`”作为下一轮 research candidate。若盘口处于 severe profile，
仓位提升必须阻断并回落到当前 sizing。
...
- 在仅用当前 live pre-submit guard 重算的数量缩放矩阵里，`1.25x` lead scale 为
  `6/6` combined pass，min scaled top-depth coverage `7468.484210`，worst
  8bp slippage headroom `2.795208bp`。`1.5x/2.0x/2.5x` 也因 testnet
  top-book coverage 极大而通过 guard。

这份 telemetry 的用途是校准 proxy 的方向，不是覆盖风险结论。它支持“`1.25x` lead scale 可以继续作为
conditional candidate”，但样本只有 6 笔且来自 testnet；同时最新一笔 entry 已出现 `5.204792bp`
adverse fill drift，距离 live `8bp` slippage guard 只剩 `2.795208bp`。因此不得因为 top-depth coverage
看起来宽就无条件放大到 `1.5x+`，更不能把 `2.5x` overlay 推成默认。下一步应继续累计生产 entry
...
- Gate 证据：
  `1.25x` live telemetry 为 `6/6` pass，strict 15bp proxy `25.554185%` 跑赢
  `lead_adverse10_exact`，strict 20bp proxy `22.278881%` 保持 kill-stress 失败，
  severe 15bp proxy `20.482920%` 失败，符合“thin-book 必须阻断放大”的预期。
- 阻塞项：
...
当前结论是：**可以进入 testnet shadow / telemetry collection；不能进入 live candidate，也不能修改默认
...
| `testnet_shadow_collect` | `sandbox=true`，ETHUSDT 1h，收集 decision/order/fill/depth telemetry | mainnet、默认 sizing 放大、跨品种推广 | live samples `>=30` 或触发 kill gate |
...
| T3 结构 / `t3_swing` | **未接入 ETH pretouch Go live 执行路径** | 当前 ETH live engine 只检测 original_t2 pretouch，并用 DT3 timing classifier + RF probability 做 skip/advance-plan 与 sizing。`t3_swing` 仍是 research overlay / portfolio 分析对象，不会触发 ETH live entry。 |
| `2.0x T3 overlay` | **research-only** | 只用于 exact-lead portfolio sensitivity、impact proxy 和 sizing readiness gate；未生成 live proposal，未改变 submitted quantity。 |
| `1.25x lead scale` | **shadow candidate，未提交为真实下单数量** | 后续 Go 阶段最多先写入 metadata / telemetry 作为 shadow-computed quantity；当前提交不放大 testnet order size。 |
| 当前 submitted sizing | **沿用 production sizing** | `suggestedQuantity = pretouchBaseOrderQuantity * pretouchBaseShare * clip(rf_probability * 2, 0, 2) * costPenalty`，`positionSizingMode=intent_quantity`。 |
| Testnet shadow | **允许进入采样阶段** | 仅用于收集 decision/order/fill/depth telemetry；readiness 状态仍是 `research_continue_collect_live_depth`，不是 live candidate。 |
...
| Event source | 当前 production-aligned lead：`pretouch_small_pullback_rf_q50_speed300_ge_q10_touch30m_eff300le1` |
| Breakout shape | 继续使用当前 live `breakout_shape_tolerance_bps=0.5` 的 restrictive 结构，不放宽 near-equal tolerance |
| Pretouch window | `pre_touch_seconds <= 1800` |
| Quality gates | `eff_300s <= 1.0`、`speed_300s_atr >= 0.228106`、`cost_q50 <= 0.116865` 后按 `pretouchCostQ50Penalty=0.50` 惩罚 |
| Model | `data/pretouch_model.json` / `20260515_v1`，Go-native DT3 timing classifier + RF probability |
| Model features | `roundtrip_cost_atr`、`prev1_range_atr`、`prev1_close_pos_side`、`level_to_signal_open_atr`、`touch_extension_atr`、`speed_300s_atr`、`eff_300s`、`pre_touch_seconds` |
| Entry action | `timing=advance-plan` 且 RF/cost sizing 有效时才生成 proposal；`skip`、未知 regime、模型缺失均 wait |
| Execution profile | 只允许 `sandbox=true` 的 Binance Futures testnet；若要观测真实 fill drift，需要显式 `auto-dispatch` 的 testnet shadow session |
| Submitted size | Shadow 初期继续提交当前 production sizing，不提交 `1.25x` 放大数量 |
| Shadow-computed size | Go 实现阶段可只在 metadata 中计算 `1.25x lead scale` 候选数量、depth gate 结果和阻断原因，用于遥测 |
| Dispatch boundary | 模板不硬编码 `dispatchMode`；调用方/前端显式传入，系统空值兜底仍为 `manual-review` |

当前 live sizing 仍以 intent 为唯一入口：
...
其中 `pretouchBaseShare=0.80`，`positionSizingMode=intent_quantity`。Shadow 阶段不得让
`defaultOrderQuantity` 覆盖 intent quantity；也不得把 research 的 `1.25x` 放大直接提交到 testnet
订单。候选放大只能作为 telemetry 字段存在，直到 readiness gate 达标。
...
| --- | ---: | ---: | ---: | ---: | --- |
| Lead same-close 上界 | `30.217222%` | - | - | - | 只作无额外成交压力上界 |
| Lead exact adverse10 | - | `22.971648%` | - | - | 保守主基线 |
| 2.0x T3 overlay + exact lead | `34.374280%` | `27.823672%` | `24.548369%` | `21.273065%` | 10-15bp 仍有 lift，20bp kill-stress 失败 |
| 1.25x lead scale + 2.0x overlay 线性 what-if | - | `33.566584%` | `30.291281%` | `27.015977%` | 没有真实 depth replay，不能直接上线 |
| 1.25x lead scale + 2.0x overlay strict impact proxy | - | - | `25.554185%` | `22.278881%` | 15bp 跑赢基线，20bp 保持 kill-stress |
| Severe thin-book proxy | - | - | `20.482920%` | - | 必须阻断放大 |

Shadow 阶段的目标收益区间可以写成：

- 主候选不是追求 `30%+` same-close，而是在 `15bp` strict impact proxy 下保持高于
  `lead_adverse10_exact=22.971648%`。
- 当前可期待的 sizing-lift 净效果约为 `25.554185% - 22.971648% = 2.582537pp`
  的 conservative proxy lift；这只是进入 testnet shadow 的理由，不是 live promotion 依据。
- 若真实 fill/depth 校准后 15bp strict proxy 回落到 `lead_adverse10_exact` 以下，或者 20bp /
...
| Gate | 当前状态 | 结论 |
| --- | --- | --- |
| Research lead 身份已锁定 | event source + model version + live template 已写入本文档 | pass |
| 保守基线可复算 | `lead_adverse10_exact=22.971648%`，62 trades，0 negative months | pass |
| Strict proxy 有 lift | strict 15bp `25.554185%` > baseline | pass |
| Kill-stress 没被误通过 | strict 20bp `22.278881%` <= baseline，severe 15bp `20.482920%` fail | pass |
| Live telemetry 方向 | `1.25x` 当前样本 `6/6` combined pass | pass for shadow |
...
2. `1.25x` shadow-computed quantity 的 combined pass ratio 必须保持 `100%`。
3. Worst `8bp` slippage headroom 必须 `>=2bp`；若出现 `adverse_fill_drift_bps >= 8`，直接 kill。
4. Strict 15bp proxy 继续高于 `lead_adverse10_exact`；strict 20bp 和 severe 15bp 继续作为 kill-stress 失败。
5. telemetry 中不得出现 `no_model_loaded`、fallback sizing、fixed default 覆盖 intent quantity、非 sandbox 下单。
...
后续写 Go 代码时，第一阶段只实现 shadow telemetry，不实现 live sizing 放大：

- 增加或复用 metadata 字段记录 current submitted quantity、shadow `1.25x` quantity、scaled top-depth coverage、
  slippage headroom、spread/book-age/source-divergence 和 block reason。
- 保持 submitted proposal quantity 等于当前 production sizing。
...
- 实现 PR 必须覆盖 `no_model_loaded`、fallback sizing、intent quantity 不被 fixed quantity 覆盖、shadow metadata
  不影响 submitted quantity、非 sandbox 不启用 shadow 放大路径等失败路径。
...
- walk-forward/monthly gate 是 breakout 结构扩展研究里的校验或候选过滤口径，不属于当前 production-aligned lead。
...
- 旧 `20260515_unified_framework_decision_report.md` 中 `22.38%/15.17%` 使用的是当时 report spec
  `trail_start_r=0.9` 的手写决策口径；当前 production-aligned lead 使用 live template 固化的
