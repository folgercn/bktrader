...
生产业务事实查询必须优先使用 `bktrader-ctl`，不要先 SSH 进服务器查 DB 或直接翻原始文件。`bktrader-ctl` 会走 platform-api 的鉴权、结构化 JSON 输出和既有只读接口，更适合作为 Agent/LLM 排查订单、成交、持仓、live session、追溯和控制面状态的事实来源。
...
排查追溯页卡显示异常时，先用 CLI 收集同一 live session 下的会话、订单、成交、持仓和订单链路：
...
1. 用 `live list` 确认目标 live session、account、symbol 和当前状态。
2. 用 `order list` / `order get` 查看 entry/exit 订单的 `status`、`reduceOnly`、metadata 里的 `liveSessionId`、`filledQuantity`、`executedQty`、`cumQty`。
3. 用 `fill list` 确认对应订单是否有成交、`exchangeTradeId`、`source`、数量、价格和手续费。
4. 用 `position list` 对照当前本地持仓是否已归零。
5. 用 `logs trace <order-id>` 串起订单生命周期、sync、settlement 和 close verification 事件。

只有在 `bktrader-ctl` 无法回答“业务事实是什么”，或需要定位进程、部署、文件日志、SSH 连通性、REST 限流等运行环境问题时，才进入下面的 SSH 日志入口。
...
## 7. LiveSession 重启 / stop-start 排障

重启实盘会话时，标准流程是先提交 stop 并等待收敛，再提交 start：

```bash
bktrader-ctl live stop <sessionId> --confirm --wait
bktrader-ctl live start <sessionId> --confirm --wait
```

如果 `stop --wait` 长时间停在 `desiredStatus=STOPPED actualStatus=RUNNING`，不要先判断为“需要 force stop”。先查控制面和 runner 是否消费了意图：

```bash
bktrader-ctl status --json
bktrader-ctl logs live-control-summary --json
```
...
- Runtime Supervisor 页面显示 `runtime supervisor is not configured` 时，它不能证明 `live-runner` 正常；仍以 `status` 和 live-control summary 为准。

## 8. 安全边界

生产排查默认只读：

- 业务数据查询优先使用 `bktrader-ctl --json`；不要绕过 API 直接改 DB。
...
- [bktrader-ctl-reference.md](bktrader-ctl-reference.md)
- [bktrader-ctl-install-deploy.md](bktrader-ctl-install-deploy.md)
