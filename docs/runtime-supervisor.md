...
- `SUPERVISOR_TARGETS` 使用逗号分隔，支持 `name=http://host:port` 或直接填写 `http(s)` base URL；显式 `name=` 形式必须有非空 name、非空 base URL，且 name 必须唯一；所有 base URL 必须使用 `http://` 或 `https://` 并包含 host，避免后续按 `targetName` 执行 suppress/backoff 控制时出现 allowlist 歧义、静默跳过 target，或把拼写错误延迟成运行期 probe 失败。
