...
- `SUPERVISOR_CONTAINER_FALLBACK_AUTO_SUBMIT=false` 为默认值；未显式设为 `true` 时，后台 collector 只生成 `containerFallbackPlan`，不会自动调用 executor。人工 `submit-container-fallback` / Dashboard submit 不受该开关阻断，仍以当前 plan 的 `executable=true` 为准。plan 会同步返回 `autoSubmitEnabled`、`autoSubmitEligible`、`manualSubmitRequired`，避免 operator 只看到 `executable=true` 却无法判断后台是否会自动提交。若设置为 `true`，配置校验要求同时启用 `SUPERVISOR_CONTAINER_RESTART_ENABLED=true` 并配置 `SUPERVISOR_CONTAINER_EXECUTOR`，避免 auto-submit policy 变成静默 no-op。
