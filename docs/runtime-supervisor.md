...
- `SUPERVISOR_CONTAINER_EXECUTOR` 默认为空，表示不配置容器 executor；当前唯一支持值是 `noop`，只用于暴露 dry-run executor readiness。`scripts/deploy.sh` 会从生产 `.env` 读取并导出该变量供 compose 插值；手动运行 compose 时，需要通过 shell/project env 显式传入。即使同时设置 `SUPERVISOR_CONTAINER_RESTART_ENABLED=true` 和 `SUPERVISOR_CONTAINER_EXECUTOR=noop`，状态也只会显示 `executorKind=noop` / `executorDryRun=true`，不会调用 Docker、不会访问 docker.sock、不会重启容器。
