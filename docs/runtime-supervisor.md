...
- `cmd/bktrader-node-agent` 是 PR3 新增的宿主机本机执行面，默认不会随 platform-api / supervisor 自动启动。它使用独立 `BKTRADER_NODE_AGENT_HTTP_ADDR`、`BKTRADER_NODE_AGENT_TOKEN` / `BKTRADER_NODE_AGENT_TOKEN_FILE`、`BKTRADER_NODE_AGENT_TARGETS_JSON` 配置，提供鉴权 `GET /v1/health` 和 `POST /v1/container-fallback/restart`。node-agent 本地 allowlist 只支持固定 `docker compose -f <composeFiles...> restart <services...>`，`projectDirectory` 必须为绝对路径，compose 文件必须是相对路径且不能 `..` 逃逸，HTTP 请求里的 `reason`、`source`、`operator` 只进入审计，不参与命令构造。Mac 当前部署仍依赖 Docker Desktop / Docker CLI / compose，建议作为宿主机手工进程或 launchd 进程运行；Linux 生产建议独立二进制 + systemd unit。完整启用、回滚、health smoke test 见 `docs/runtime-supervisor-production-executor-design.md` 的 PR3 部署 runbook。
