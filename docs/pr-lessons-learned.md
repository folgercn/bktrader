...
> **来源**：从项目 167 个 PR（#1 ~ #167）的 review 记录、comments、diff 中提炼。
> **维护者**：folgercn
> **最后更新**：2026-04-23
...
**典型案例**：PR#167（guard live float tolerance and alert exit failures）

`0.013` 这类本来满足 Binance `stepSize=0.0001` 的数量，在 Go `float64` 里会出现 `0.013000000000000001` 的尾差。此前 live 下单归一化、本地 reduce-only 校验、fill settlement、position apply 等链路各自维护 `1e-9` / `1e-12` / `math.Abs(...)`，导致：

- reduce-only 平仓单在 submit 前被误判为"扩量"并本地 reject
- 本地订单数量和交易所归一化数量不一致，引发 order sync 风暴
- terminal filled backfill 把已满单当成未满
- 本地持仓全平后仍残留极小数量，或被误判成反向开仓

**修复模式**：

- 引入统一精度入口 `internal/service/precision_tolerance.go`
- 订单数量比较统一走 `tradingQuantity*` helper
- 本地价格一致性比较统一走 `tradingPrice*` helper
- 交易所 `stepSize` / `tickSize` 比较统一走 `exchangeIncrement*` helper
- `minNotional` 边界统一走 `exchangeNotionalSatisfiesMinimum(...)`
- `round` / `ceil` 也必须走统一 increment helper，不允许在业务函数里散落 `+1e-9` / `-1e-12`
- 回归测试必须同时覆盖"尾差应容忍"和"真实超量/短缺仍要拒绝"

**规则提炼**：
> 实盘执行链路禁止散落浮点容差常量。任何数量、价格、notional 的比较 / round / ceil，都必须走统一 precision helper；新增交易所规则时必须显式定义 absolute tolerance、relative tolerance、单位 scale，以及边界测试。
