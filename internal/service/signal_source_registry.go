...
func (p *Platform) registerBuiltInSignalSources() {
	...
	register(staticSignalSourceProvider{definition: domain.SignalSourceDefinition{
		...
		Description:  "交易所原生 5m/15m/30m/1h/4h/1d K 线流，用于实时策略信号计算、SMA5/MA20 和前三根 OHLC 状态。",
...
func (p *Platform) SignalSourceCatalog() map[string]any {
	...
	return map[string]any{
		...
		"notes": []string{
			...
			"5m / 15m / 30m / 1h / 4h / 1d 策略信号建议直接绑定交易所 kline 源，而不是从 tick 聚合 signal bar。",
	...
	return []map[string]any{
		{
			"streamType":    "signal_bar",
			"primaryRole":   "signal",
			"description":   "交易所原生 K 线流，适合作为 5m / 15m / 30m / 1h / 4h / 1d 策略信号源。",
