...
func buildEthPretouchTimingTemplate(strategyID, strategyName, strategyVersionID string) LiveLaunchTemplate {
	...
	liveOverrides := map[string]any{
		...
		"pretouchShadowOverlayQualityMinMultiplier":    defaultPretouchShadowOverlayQualityMin,
		"pretouchShadowOverlayQualityMaxMultiplier":    defaultPretouchShadowOverlayQualityMax,
		"pretouchShadowOverlayQualityMinQuantity":      defaultPretouchShadowOverlayQualityMinQty,
		"pretouchShadowOverlayQualityMaxQuantity":      defaultPretouchShadowOverlayQualityMaxQty,
	...
	return LiveLaunchTemplate{
		...
		Notes: []string{
			...
			"Research shadow 口径：lead 0.20-0.40 ETH quantity band + overlay 2.0x + T3 RF/cost 0.20-0.40 ETH quantity band；T3 quantity-band exact-lead evidence 为 68.610750%，比 fixed overlay +34.236470pp。",
			...
			"Lead 模型不可用或校验失败时 lead 事件自动 skip；T3 quality 模型不可用时只回落 fixed overlay 并写 metadata，不扩大 mainnet 行为。",
